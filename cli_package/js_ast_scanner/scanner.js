 ...
 *   exit 1 : parse / setup error  →  vibecheck falls back to regex
 *
  ...
  lines.forEach((line, i) => {
    ...
    if (m) {
      const ids = m[1]
        ...
        : new Set(["*"]);
      // Apply to the line the comment is on (inline: `code // vibecheck-ignore`)
      map[i + 1] = ids;
