...
def check_repo(path):
    repo_dir = Path(path) / ".codesentry-prompts"
    if not repo_dir.exists():
        print("Not a codesentry prompt repo. Run: codesentry prompt init")
        sys.exit(1)
    return repo_dir
...
def commit(message, path):
    ...
    if not config.get("tracked_files"):
        print("No files tracked. Run: codesentry prompt add <file> <path>")
        sys.exit(1)
...
@click.argument('path', default='.', type=click.Path())
def diff(ref_a, ref_b, path):
    """Show what changed between two prompt versions.

    \b
    Examples:
      codesentry prompt diff v1 v2
      codesentry prompt diff v2 HEAD
      codesentry prompt diff 9f8e7d6c a1b2c3d4
    """
    import difflib
    ...
    a_data, a_id = resolve_ref(ref_a, history, commits_dir)
    b_data, b_id = resolve_ref(ref_b, history, commits_dir)

    if not a_data or not b_data:
        print("Could not resolve one or both refs.")
        sys.exit(1)

    all_files = sorted(
        set(list(a_data.get("files", {}).keys()) + list(b_data.get("files", {}).keys()))
    )

    any_diff = False
    for fname in all_files:
        a_content = a_data["files"].get(fname, "")
        b_content = b_data["files"].get(fname, "")
        ...
        a_words  = len(a_content.split())
        b_words  = len(b_content.split())
        delta    = b_words - a_words
        sign     = "+" if delta >= 0 else ""

        print(f"\n--- {ref_a}/{fname}  ({a_words} words)")
        print(f"+++ {ref_b}/{fname}  ({b_words} words, {sign}{delta})")
        print(f"    [{a_data['timestamp']}] {a_data['message']}")
        print(f"    [{b_data['timestamp']}] {b_data['message']}")
        print()

        for line in difflib.unified_diff(
            a_lines, b_lines,
            fromfile=f"{ref_a}/{fname}",
            tofile=f"{ref_b}/{fname}",
            lineterm="",
            n=3,
        ):
            # Colour-code additions / removals in terminals that support it
    ...
    if not any_diff:
        print(f"No differences between {ref_a} ({a_id[:8]}) and {ref_b} ({b_id[:8]}).")
