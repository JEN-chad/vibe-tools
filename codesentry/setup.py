"""
codesentry/setup.py — API key management & onboarding
Handles load_api_key(), save_global_key(), save_local_key(), interactive_setup()
and the `codesentry setup` CLI command.
"""
...
def _prompt_for_scope(console: Console) -> str:
    ...
    console.print("  [bold]1.[/bold] Global (recommended) — [dim]~/.codesentry/config[/dim]")
...
def _check_existing_key(console: Console, scope: str) -> bool:
    ...
    console.print(
        ...
        f"{'~/.codesentry/config' if scope == 'global' else '.vibe_env'}.[/yellow]"
    )
...
def interactive_setup(console: Console, scope: str | None = None) -> bool:
    ...
    """
    console.print(Panel(
        "[bold cyan]codesentry Setup[/bold cyan]\n"
...
@click.command("setup")
@click.option("--global", "scope", flag_value="global", help="Store API key globally (~/.codesentry/config)")
@click.option("--local",  "scope", flag_value="local",  help="Store API key in current project (.vibe_env)")
def setup_cmd(scope):
    """
    Configure your GROQ API key for codesentry.

    \b
    Storage locations:
      --global   ~/.codesentry/config   (used across all projects)
