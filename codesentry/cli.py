"""
codesentry — unified CLI entry point

  codesentry check       → scan code for issues
  codesentry prompt      → version prompt files
  codesentry hook        → manage the global git pre-commit hook
  codesentry setup       → configure GROQ API key (global or local)
"""
...
from codesentry.hook import hook
from codesentry.setup import setup_cmd
...
def cli(ctx):
    """
     codesentry — AI code safety & prompt versioning toolkit
...
def check(path, autofix, copy, one_prompt, strict, compact, narrate, explain,
    ...
    from codesentry.vibecheck import run_check, ScanConfig
...
def prompt_cmd(args):
    ...
    """
    import sys
    from codesentry.promptgit import cli as promptgit_cli
...
def check_entry():
    from codesentry.vibecheck import main
    main()


def promptgit_entry():
    from codesentry.promptgit import cli
    cli()